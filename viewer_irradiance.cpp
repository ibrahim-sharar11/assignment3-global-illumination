...
int triangles;
GLuint envMap;
GLuint irradianceMap;  // Added for irradiance map
...
void init() {
	...
	*/
...
#else
	texture = loadCube((char*)"VancouverConventionCentre");
#endif
	...
	/*
	 *  Load the irradiance map (blurred environment map)
	 *  For Part 2, you need to create this by blurring the environment map
	 */
#ifdef WIN32
	texture = loadCube("../../VancouverConventionCentreIrradiance");
#else
	// Try to load irradiance map, fall back to env map if not found
	texture = loadCube((char*)"VancouverConventionCentreIrradiance");
#endif
	if (texture == NULL) {
		// If irradiance map doesn't exist, use environment map as fallback
		printf("Warning: Irradiance map not found. Using environment map.\n");
		texture = loadCube((char*)"VancouverConventionCentre");
	}
...
void display(void) {
	...
	// Bind environment map to texture unit 0
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, envMap);
	texLoc = glGetUniformLocation(program, "tex");
	glUniform1i(texLoc, 0);
	
	// Bind irradiance map to texture unit 1
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_CUBE_MAP, irradianceMap);
	texLoc = glGetUniformLocation(program, "irradianceMap");
	if (texLoc != -1) {
...
int main(int argc, char **argv) {
	 ...
	 */
	sprintf(vertexName, "example12%s.vs", vertex);
	vs = buildShader(GL_VERTEX_SHADER, vertexName);
	sprintf(fragmentName, "example12%s.fs", fragment);
	fs = buildShader(GL_FRAGMENT_SHADER, fragmentName);
	program = buildProgram(vs, fs, 0);
	vs = buildShader(GL_VERTEX_SHADER, (char*)"cube.vs");
	fs = buildShader(GL_FRAGMENT_SHADER, (char*)"cube.fs");
